...
public class CommonExcelReaderService {
    ...
    public ExcelCommonV2 getCommonExcelDataV2(String filePath) {
        // 待组装的对象
        ExcelCommonV2 data = new ExcelCommonV2();
        try {
            // 获取表格
            Sheet sheet = ExcelReader.getFirstSheet(filePath);
            // 获取第一行的属性
            ...
            if (sheet != null) {
                ...
                for (int i = 0; i < headNameList.size(); i++) {
                    ...
                    // 列属性
                    cellData.setProperty(headNameList.get(i));
                    // 列单元值
                    List<String> cellValues = new ArrayList<>();
                    // 从第2行开始
                    for (int j = 1; j < rowCount; j++) {
                    ...
                    cellData.setCellValue(cellValues);
                    cellDataList.add(cellData);
                } // 遍历列结束
                data.setCellData(cellDataList);
            } // 处理表格内容完成
